...
async function setupPage() {
  ...
  if (privacyPopup && acceptPrivacyBtn) {
    ...
    const hasAcceptedPrivacy = localStorage.getItem(
      "infoSuplementosPrivacyAccepted"
    );
  ...
  if (isLoggedIn && user.tipo === "ADMINISTRADOR") {
    ...
    li.id = "admin-link-li";
    li.innerHTML =
      '<a href="admin.html" style="color: #e67e22; font-weight: bold;">Admin</a>';
  ...
  if (grid) {
    grid.addEventListener("click", async (event) => {
      ...
      if (!isLoggedIn) {
        alert(
          "É necessário fazer o login para adicionar produtos aos favoritos."
        );
        return;
      }
