document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        const response = await fetch('/api/infosuplementos/favoritos', { credentials: 'include' });
            ...
            favorites.forEach(product => {
                ...
                card.innerHTML = `
                    ...
                    <div class="card-footer">
                        <button class="remove-favorite-btn" data-id="${product.id_suplemento}">Remover</button>
                    </div>
                `;
...
document.getElementById('favorites-grid').addEventListener('click', async (event) => {
    if (event.target.classList.contains('remove-favorite-btn')) {
        ...
        try {
            const response = await fetch(`/api/infosuplementos/favoritos/${suplementoId}`, {
                method: 'DELETE',
                credentials: 'include'
            });
